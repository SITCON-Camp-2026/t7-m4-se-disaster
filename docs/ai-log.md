...
| 2026-07-09 | v1 修正    | 強化 v1 密碼門檻狀態             | 建議將 v1 密碼門檻抽成 `V1AccessGate`，並用 `sessionStorage` 記錄同分頁已通過狀態                                             | 採用        | 新分頁或新 session 仍需輸入密碼；這仍是前端 demo 門檻，不是正式安全驗證                      | `src/app/App.tsx`, `tests/app-smoke.test.tsx`, `docs/ai-log.md`                                                     |
